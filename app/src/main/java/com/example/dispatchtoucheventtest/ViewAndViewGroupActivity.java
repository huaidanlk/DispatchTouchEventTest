...
public class ViewAndViewGroupActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        * 如果 子View 不处理改事件 即 view.dispatchTouchEvent() 返回为false ,那么 就会 让其 父viewGroup
        * 来出事改事件，同样调用的是 view.dispatchTouchEvent()。
        * 如果 view都不对该事件做处理，则会传递个activity 来处理，会调用 activity.onTouchEvent() 来处理
        * */

        /*测试 MyLayout onInterceptTouchEvent return false ,不拦截事件

                当点击空白区域的时，MyLayout dispatchTouchEvent
        ...
        *      当点击button 时  MyLayout dispatchTouchEvent
                ...
                OnTouchListener 事件并在onTouch()中返回了true,则 onClick 事件会被拦截
                如果 返回了 false 则 onClick 不会被拦截
                此时 view.dispatchTouchEvent() 仍然会返回 true 即事件被消耗了
                这是因为 尽管在onTouch()中返回了false 但是仍然会进入 onTouchEvent()函数中，
                因为button的 是可以点击的，所以 onTouchEvent()一定会返回true ,即button 消耗了事件
...
//                return true;//返回true后 不会执行 onTouchEvent() onClick 事件无法触发
                return false;  //返回true后 会执行 onTouchEvent() onClick 事件可以触发
            }
        });
